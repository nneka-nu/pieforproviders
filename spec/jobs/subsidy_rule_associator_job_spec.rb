...
RSpec.describe SubsidyRuleAssociatorJob, type: :job do
  describe '#perform' do
    let(:child) { create(:child) }
    let(:service) { instance_double(SubsidyRuleAssociator) }

    before { allow(SubsidyRuleAssociator).to receive(:new).and_return(service) }
